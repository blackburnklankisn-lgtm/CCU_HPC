...
[!// ====================== ERROR CHECKS FOR StbMTimeLeapFutureThreshold AND StbMTimeLeapPastThreshold PARAMETERS ======================
[!LOOP "node:order(as:modconf('StbM')[1]/StbMSynchronizedTimeBase/*[not(node:value(./StbMSynchronizedTimeBaseIdentifier) > 31)], './StbMSynchronizedTimeBaseIdentifier')"!]
    ...
    [!// a MASTER time base shall not have StbMTimeLeapFutureThreshold or StbMTimeLeapPastThreshold activated
    [!IF "($isMasterConnected = 'isMaster')"!]
      [!IF "((node:exists(StbMTimeLeapFutureThreshold)) or (node:exists(StbMTimeLeapPastThreshold)))"!]
        [!// Time leap supervision is only done for slave or gateway time bases
...
[!// ====================== ERROR CHECKS FOR StbM SharedData_V2 ======================
...
[!IF "((node:exists(as:modconf('StbM')[1]/StbMGeneral/StbMProvideDataInSharedMemoryVersion) = 'true') and (node:value(as:modconf('StbM')[1]/StbMGeneral/StbMProvideDataInSharedMemoryVersion) = 'V2'))"!]
  ...
  [!LOOP "node:order(as:modconf('StbM')[1]/StbMSynchronizedTimeBase/*, './StbMSynchronizedTimeBaseIdentifier')"!]
    ...
    [!CALL "GetConnectedBusTSynModule", "timeBase" = "node:path(.)"!]
	  ...
	  [!/* !LINKSTO StbM.EB.SharedMemorySupport_V2.Check.StbMProvideDataInSharedMemory.OnSLAVE,1 */!]
    [!// a slave time base can not not have StbMProvideDataInSharedMemory configured to 'CONSUMER' 
    [!IF "(($isSlaveConnected = 'isSlave') and (node:value(./StbMProvideDataInSharedMemory) = 'CONSUMER'))"!]
      [!ERROR!] SLAVE timeBase with ID [!"$TimeBaseId"!] cannot have StbMProvideDataInSharedMemory set to CONSUMER![!ENDERROR!]
    [!ENDIF!]
  [!ENDLOOP!]
  ...
  [!LOOP "node:order(as:modconf('StbM')[1]/StbMSynchronizedTimeBase/*, './StbMSynchronizedTimeBaseIdentifier')"!]
    ...
    [!CALL "GetConnectedBusTSynModule", "timeBase" = "node:path(.)"!]
	  ...
	  [!/* !LINKSTO StbM.EB.SharedMemorySupport_V2.Check.StbMProvideDataInSharedMemory.OnGATEWAY,1 */!]
    [!// a gateway time base can not not have StbMProvideDataInSharedMemory configured to 'CONSUMER' 
    [!IF "(($isMasterSlaveConnected = 'true') and (node:value(./StbMProvideDataInSharedMemory) = 'CONSUMER'))"!]
      [!ERROR!] GATEWAY timeBase with ID [!"$TimeBaseId"!] cannot have StbMProvideDataInSharedMemory set to CONSUMER![!ENDERROR!]
    [!ENDIF!]
  [!ENDLOOP!]
  ...
  [!/* !LINKSTO StbM.EB.SharedMemorySupport_V2.Check.PROVIDERs_And_CONSUMERs_NotAllowed_InTheSameConfiguration,1 */!]
  [!// check that StbM has no PROVIDERs and CONSUMERs present in the same configuration 
  [!IF "(($StbMSharedDataProviderTimeBaseUsed = 'true') and ($StbMSharedDataConsumerTimeBaseUsed = 'true'))"!]
    [!ERROR!] PROVIDERs and CONSUMERs are not allowed in the same StbM configuration![!ENDERROR!]
  [!ENDIF!]


  [!// this validation is relevant for CONSUMERs only
  [!IF "($StbMSharedDataConsumerTimeBaseUsed = 'true')"!]
  
  [!// at least one time base needs to share data --> enable data sharing
  [!VAR "StbMShareDataConsumerTimeBasesNumber" = "num:i(0)"!]
  [!LOOP "StbMSynchronizedTimeBase/*"!]
  [!IF "node:value(./StbMProvideDataInSharedMemory) = 'CONSUMER'"!]
    [!VAR "StbMShareDataConsumerTimeBasesNumber" = "num:i($StbMShareDataConsumerTimeBasesNumber + 1)"!]
  [!ENDIF!]
  [!ENDLOOP!]
  [!/* !LINKSTO StbM.EB.SharedMemorySupport_V2.Check.StbMSharedDataTimeBaseIndex_NotAllowedToBe_GreaterThan_TotalNumberOfSharedDataTimeBases,1 */!]
  [!// check if there is at least one time base which has StbMSharedDataTimeBaseIndex greater or equal to StbMShareDataConsumerTimeBasesNumber
  [!LOOP "StbMSynchronizedTimeBase/*"!]
  [!IF "((node:exists(./StbMSharedDataTimeBaseIndex) = 'true') and (node:value(./StbMSharedDataTimeBaseIndex) >= $StbMShareDataConsumerTimeBasesNumber))"!]
    [!ERROR!] StbMSharedDataTimeBaseIndex for timeBase with ID [!"$TimeBaseId"!] shall be smaller than the total number of time bases which have StbMProvideDataInSharedMemory set to CONSUMER![!ENDERROR!]
    [!BREAK!]
  [!ENDIF!]
  [!ENDLOOP!]
