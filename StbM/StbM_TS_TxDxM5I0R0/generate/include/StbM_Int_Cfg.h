...
/* !LINKSTO StbM.SWS_StbM_00426,1 */
#include <Gpt.h>
[!ENDIF!]

/* !LINKSTO StbM.SWS_StbM_00065,1 */
...
#if (defined STBM_NUMBER_OF_INDEXES_FOR_VERSION_AND_TIMEBASENUMBER)
#error STBM_NUMBER_OF_INDEXES_FOR_VERSION_AND_TIMEBASENUMBER is already defined.
#endif
/** \brief Number of shared data array indexes needed for version and timeBaseNumber */
#define STBM_NUMBER_OF_INDEXES_FOR_VERSION_AND_TIMEBASENUMBER           3U
...
#if (defined STBM_SHARED_DATA_VERSION)
#error STBM_SHARED_DATA_VERSION is already defined.
#endif
/** \brief Which version of shared data is used */
#define STBM_SHARED_DATA_VERSION                [!"$StbMSharedDataVersion"!]U
[!ELSE!]
#if (defined STBM_SHARED_DATA_VERSION)
#error STBM_SHARED_DATA_VERSION is already defined.
#endif
/** \brief Which version of shared data is used */
#define STBM_SHARED_DATA_VERSION                0U
[!ENDIF!]


#if (defined STBM_SHARED_DATA_V1)
#error STBM_SHARED_DATA_V1 is already defined.
#endif
/** \brief Shared data version 1 */
#define STBM_SHARED_DATA_V1                1U

#if (defined STBM_SHARED_DATA_V2)
#error STBM_SHARED_DATA_V2 is already defined.
#endif
/** \brief Shared data version 2 */
#define STBM_SHARED_DATA_V2                2U
...
[!IF "($StbMSharedDataVersion = '2')"!]
/** \brief Version start index in SharedData array */
#define STBM_SHAREDDATA_VERSION_STARTINDEX 0U
/** \brief TimeBaseNumber start index in SharedData array */
#define STBM_SHAREDDATA_TIMEBASENUMBER_STARTINDEX 1U

/** \brief GlobalTime start index in SharedData array */
#define STBM_SHAREDDATA_GLOBALTIME_STARTINDEX 0U
/** \brief VirtualLocalTime start index in SharedData array */
#define STBM_SHAREDDATA_VIRTUALLOCALTIME_STARTINDEX 11U
/** \brief RateDeviation start index in SharedData array */
#define STBM_SHAREDDATA_RATEDEVIATION_STARTINDEX 19U
/** \brief RateCorrection start index in SharedData array */
#define STBM_SHAREDDATA_RATECORRECTION_STARTINDEX 21U
/** \brief TimeBaseId start index in SharedData array */
#define STBM_SHAREDDATA_TIMEBASEID_STARTINDEX 25U
/** \brief BeginCounter start index in SharedData array */
#define STBM_SHAREDDATA_BEGINCOUNTER_STARTINDEX 26U
/** \brief EndCounter start index in SharedData array */
#define STBM_SHAREDDATA_ENDCOUNTER_STARTINDEX 27U

/** \brief Number of shared data bytes per each timebase */
#define STBM_NUMBER_OF_SHAREDDATA_BYTES_PER_TIMEBASE 28U

/** \brief Byte mask used when sharing current timebase data for uint32 types */
#define STBM_32BIT_MASK0_LSB (uint32) 0x000000FFU
#define STBM_32BIT_MASK1 (uint32) 0x0000FF00U
#define STBM_32BIT_MASK2 (uint32) 0x00FF0000U
#define STBM_32BIT_MASK3_MSB (uint32) 0xFF000000U

/** \brief Byte mask used when sharing current timebase data for uint16 types */
#define STBM_16BIT_MASK0_LSB (uint16) 0x00FFU
#define STBM_16BIT_MASK1_MSB (uint16) 0xFF00U

#if (defined STBM_SHAREDDATA_PROVIDER_TIMEBASES_USED)
#error STBM_SHAREDDATA_PROVIDER_TIMEBASES_USED is already defined.
#endif
/** \brief Enables/disables writing in shared memory for provider timebases */
#define STBM_SHAREDDATA_PROVIDER_TIMEBASES_USED       STD_[!IF "$StbMSharedDataProviderTimeBaseUsed = 'true'"!]ON[!ELSE!]OFF[!ENDIF!]

#if (defined STBM_SHAREDDATA_CONSUMER_TIMEBASES_USED)
#error STBM_SHAREDDATA_CONSUMER_TIMEBASES_USED is already defined.
#endif
/** \brief Enables/disables reading from shared memory for consumer timebases */
#define STBM_SHAREDDATA_CONSUMER_TIMEBASES_USED       STD_[!IF "$StbMSharedDataConsumerTimeBaseUsed = 'true'"!]ON[!ELSE!]OFF[!ENDIF!]
...
#if (defined STBM_SYNCMASTER_LINKEDTO_ETHTSYN_TIMEDOMAIN)
#error STBM_SYNCMASTER_LINKEDTO_ETHTSYN_TIMEDOMAIN is already defined.
#endif
/** \brief Whether there are MASTER timebases linked to EthTSyn time domains configured */
#define STBM_SYNCMASTER_LINKEDTO_ETHTSYN_TIMEDOMAIN       STD_[!IF "$syncMasterTimeBasesLinkedToEthTSynTimeDomain = 'true'"!]ON[!ELSE!]OFF[!ENDIF!]

#if (defined STBM_SYNCSLAVE_LINKEDTO_ETHTSYN_TIMEDOMAIN)
#error STBM_SYNCSLAVE_LINKEDTO_ETHTSYN_TIMEDOMAIN is already defined.
#endif
/** \brief Whether there are SLAVE timebases linked to EthTSyn time domains configured */
#define STBM_SYNCSLAVE_LINKEDTO_ETHTSYN_TIMEDOMAIN       STD_[!IF "$syncSlaveTimeBasesLinkedToEthTSynTimeDomain = 'true'"!]ON[!ELSE!]OFF[!ENDIF!]
...
[!VAR "StbMOsCounter" = "as:modconf('StbM')[1]/StbMSynchronizedTimeBase/*[not(node:empty(./StbMLocalTimeClock/StbMLocalTimeHardware))]/StbMLocalTimeClock/StbMLocalTimeHardware[node:exists(node:ref(.)/../../OsCounter)]"!]
...
/** \brief This typedef defines the version1 - type of the Shared Memory of one time base
 */
typedef struct
{
...
typedef struct
{
  VAR(uint32, TYPEDEF) Version;  /**< Current Version of this type; */
...
typedef struct
{
  ...
  CONST(uint8, TYPEDEF) isTimeBaseLinkedToEthTSynTimeDomain;  /**< Indicates if time base is linked or not to EthTSyn time domain */
...
/** \brief source time base config
 **
 ** This type contains the precompile time configuration
 ** of an source time base
 */
typedef struct
{
  CONST(StbM_SynchronizedTimeBaseType, TYPEDEF) syncTimeBaseIndex;  /**< Index of the related synchronized time base */
  CONST(uint8, TYPEDEF) haveSameVirtualSources;  /**< relation between the src and dest virtual time sources */

} StbM_SourceTimeBaseCfgType;
...
typedef struct
{
  ...
  CONST(uint32, TYPEDEF) sharedDataTimeoutValue;  /**< shared data timeout of time base in MainFunction invocations */
...
#if (STBM_TIME_BASES_THAT_SHARE_DATA_USED == STD_ON)
  CONST(uint16, TYPEDEF) timeBaseSharesDataIdx;  /**< index for time bases in the StbM_SharedEthOffsetData.Offset[] array */
#if (STBM_SHARED_DATA_VERSION == STBM_SHARED_DATA_V2)
  CONST(uint8, TYPEDEF) timeBaseRoleSharedData;  /**< role of the time base when sharing data */
#endif /* (STBM_SHARED_DATA_VERSION == STBM_SHARED_DATA_V2) */
#endif /* STBM_TIME_BASES_THAT_SHARE_DATA_USED == STD_ON */
...
/** \brief CloneData for each a destination time base
 **
 ** This type is used to store srcTimeBaseId and cloneCfg at run-time
 ** for each a destination time base that has configured StbMSourceTimeBase.
 */
typedef struct
{
  VAR(uint8, TYPEDEF) srcTimeBaseId;  /**< Source Time Base Id */
  VAR(StbM_CloneConfigType, TYPEDEF) cloneCfg;  /**<Clone Config value */
} StbM_CloneDataType;
#endif /* (STBM_SOURCE_TIMEBASES_USED == STD_ON) */
...
#if (STBM_SOURCE_TIMEBASES_USED == STD_ON)
/** \brief Source time bases precompile time configuration structure */
extern CONST(StbM_SourceTimeBaseCfgType, STBM_APPL_CONST) StbM_SourceTimeBaseCfg[STBM_NUMBER_OF_SYNC_TIMEBASES + STBM_NUMBER_OF_OFFSET_TIMEBASES+ STBM_NUMBER_OF_PURE_TIMEBASES];
#endif /* (STBM_SOURCE_TIMEBASES_USED == STD_ON) */

#if (STBM_NUMBER_OF_SOURCE_TIMEBASES == STD_ON)
/** \brief Source time bases precompile time configuration structure */
extern CONST(StbM_SourceTimeBaseCfgType, STBM_APPL_CONST) StbM_SourceTimeBaseCfg[STBM_NUMBER_OF_SYNC_TIMEBASES + STBM_NUMBER_OF_OFFSET_TIMEBASES+ STBM_NUMBER_OF_PURE_TIMEBASES];
#endif /* #if (STBM_NUMBER_OF_SOURCE_TIMEBASES == STD_ON) */
...
extern VAR(StbM_SharedEthOffsetType, STBM_VAR_CLEARED) StbM_SharedEthOffsetData;
...
extern VAR(uint8, STBM_VAR_CLEARED) StbM_SharedData[STBM_NUMBER_OF_INDEXES_FOR_VERSION_AND_TIMEBASENUMBER + (STBM_NUMBER_TIME_BASES_THAT_SHARE_DATA_MODULES*STBM_NUMBER_OF_SHAREDDATA_BYTES_PER_TIMEBASE)];
