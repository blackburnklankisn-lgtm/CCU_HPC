...
[!LOOP "node:order(as:modconf('StbM')[1]/StbMSynchronizedTimeBase/*, './StbMSynchronizedTimeBaseIdentifier')"!]
  ...
  [!VAR "isTimeRecordingUsedOnTimeBase_NoCallbackConfigured" = "false()"!]
  ...
  [!IF "node:exists(./StbMTimeRecording)"!]
    ...
    [!IF "$TimeBaseId < 16"!]
	  [!VAR "isTimeRecordingUsedOnTimeBase_NoCallbackConfigured" = "((node:value(./StbMTimeRecording/StbMSyncTimeRecordTableBlockCount) > 0) and (not(node:exists(./StbMTimeRecording/StbMSyncTimeRecordBlockCallback))))"!]
    [!ELSE!]
      [!VAR "isTimeRecordingUsedOnTimeBase_NoCallbackConfigured" = "((node:value(./StbMTimeRecording/StbMOffsetTimeRecordTableBlockCount) > 0) and (not(node:exists(./StbMTimeRecording/StbMOffsetTimeRecordBlockCallback))))"!]
    [!ENDIF!]
  [!ENDIF!]
