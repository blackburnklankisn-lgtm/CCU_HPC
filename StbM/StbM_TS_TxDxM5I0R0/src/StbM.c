 ...
 * W589: pointer assumed to be nonzero - test removed violated
 *
 * Reason:
...
/* patch version check */
#if (STBM_SW_PATCH_VERSION < 0U)
#error STBM_SW_PATCH_VERSION wrong (< 0U)
#endif
...
{
  DBG_STBM_UPDATEGLOBALTIMEINSHAREDMEMORY_ENTRY(commonIndex, timeBaseStartIndex);

  /* !LINKSTO StbM.EB.SharedMemorySupport_V2.TimeProvider.UpdateSharedVariable.CurrentTimeBaseData.TimeBaseStatus,1 */
  /* update the status */
  StbM_SharedData[timeBaseStartIndex + STBM_SHAREDDATA_GLOBALTIME_STARTINDEX + 0U] = StbM_TimeStamps[commonIndex].globalTime.timeBaseStatus;
  ...
  DBG_STBM_UPDATEGLOBALTIMEINSHAREDMEMORY_EXIT(commonIndex, timeBaseStartIndex);
}
...
{
  DBG_STBM_UPDATEVIRTUALLOCALTIMEINSHAREDMEMORY_ENTRY(commonIndex, timeBaseStartIndex);

  /* !LINKSTO StbM.EB.SharedMemorySupport_V2.TimeProvider.UpdateSharedVariable.CurrentTimeBaseData.TimeTuple,1 */
  /* update VLT */
  /* VLT - nanosecondsLo */
  StbM_SharedData[timeBaseStartIndex + STBM_SHAREDDATA_VIRTUALLOCALTIME_STARTINDEX + 0U] = (uint8)((uint32)(StbM_TimeStamps[commonIndex].virtualLocalTime.nanosecondsLo & STBM_32BIT_MASK3_MSB) >> 24U);
  ...
  DBG_STBM_UPDATEVIRTUALLOCALTIMEINSHAREDMEMORY_EXIT(commonIndex, timeBaseStartIndex);
}
...
{
  DBG_STBM_UPDATETIMETUPLEINSHAREDMEMORY_ENTRY(commonIndex, timeBaseStartIndex);
  ...
  DBG_STBM_UPDATETIMETUPLEINSHAREDMEMORY_EXIT(commonIndex, timeBaseStartIndex);
}
...
)
{
  ...
  DBG_STBM_UPDATERATEDEVANDRATECORRINSHAREDMEMORY_ENTRY(rateCorrection, rateDeviation, timeBaseStartIndex);
  ...
  /* update rate deviation */
  StbM_SharedData[timeBaseStartIndex + STBM_SHAREDDATA_RATEDEVIATION_STARTINDEX + 0U] = (uint8)((uint16)(tempRateDev & STBM_16BIT_MASK1_MSB) >> 8U);
  ...
  StbM_SharedData[timeBaseStartIndex + STBM_SHAREDDATA_RATECORRECTION_STARTINDEX + 0U] = (uint8)((uint32)(*rateCorrection & STBM_32BIT_MASK3_MSB) >> 24U);
  ...
  DBG_STBM_UPDATERATEDEVANDRATECORRINSHAREDMEMORY_EXIT(rateCorrection, rateDeviation, timeBaseStartIndex);
}
#endif /* ((STBM_MASTER_RATE_CORRECTION_USED == STD_ON) || (STBM_SLAVE_RATE_CORRECTION_USED == STD_ON)) */
...
{
  DBG_STBM_READGLOBALTIMEFROMSHAREDMEMORY_ENTRY(commonIndex, timeBaseStartIndex);
   ...
   StbM_TimeStamps[commonIndex].globalTime.timeBaseStatus = StbM_SharedData[timeBaseStartIndex + STBM_SHAREDDATA_GLOBALTIME_STARTINDEX + 0U];
  ...
  DBG_STBM_READGLOBALTIMEFROMSHAREDMEMORY_EXIT(commonIndex, timeBaseStartIndex);
}
...
{
  DBG_STBM_READVIRTUALLOCALTIMEFROMSHAREDMEMORY_ENTRY(commonIndex, timeBaseStartIndex);

   /* !LINKSTO StbM.EB.SharedMemorySupport_V2.TimeConsumer.UseSharedVariable.CurrentTimeBaseData.TimeTuple,1 */
   /* save current VLT values for the SYNC time base */
   StbM_TimeStamps[commonIndex].virtualLocalTime.nanosecondsLo = (uint32)(((uint32)(StbM_SharedData[timeBaseStartIndex + STBM_SHAREDDATA_VIRTUALLOCALTIME_STARTINDEX + 0U]) << 24) | \
  ...
  DBG_STBM_READVIRTUALLOCALTIMEFROMSHAREDMEMORY_EXIT(commonIndex, timeBaseStartIndex);
}
...
)
{
  ...
  DBG_STBM_READDATAFROMSHAREDMEMORY_ENTRY(commonIndex, timeBaseStartIndex);
   ...
   if(STBM_MASTER_RATE_CORRECTION_DISABLED(commonIndex) == FALSE)
   {
     ...
     lclMasterRateDeviation = (uint16)(((uint16)(StbM_SharedData[timeBaseStartIndex + STBM_SHAREDDATA_RATEDEVIATION_STARTINDEX + 0U]) << 8) | \
     ...
     if(tempMasterRateDev != STBM_RATE_DEVIATION_INVALID_VALUE)
     {
       ...
       /* update rate correction */
       StbM_MasterRateCorrectionData[rcDataIndex].rrc = (uint32)(((uint32)(StbM_SharedData[timeBaseStartIndex + STBM_SHAREDDATA_RATECORRECTION_STARTINDEX + 0U]) << 24) | \
  ...
  DBG_STBM_READDATAFROMSHAREDMEMORY_EXIT(commonIndex, timeBaseStartIndex);
}
#endif /* (STBM_SHAREDDATA_CONSUMER_TIMEBASES_USED == STD_ON) */
...
/* Init the clone structure*/
      ...
      if(sharedMemoryIndex != STBM_TIME_BASE_DOES_NOT_USE_SHARED_DATA)
      {
        ...
        /* TG - timeBaseStatus */
        StbM_SharedData[timeBaseStartIndex + STBM_SHAREDDATA_GLOBALTIME_STARTINDEX + 0U] = 0x00U;
        /* TG - nanoseconds */
        ...
        /*  VLT - nanosecondsLo */
        StbM_SharedData[timeBaseStartIndex + STBM_SHAREDDATA_VIRTUALLOCALTIME_STARTINDEX + 0U] = 0x00U;
        ...
        StbM_SharedData[timeBaseStartIndex + STBM_SHAREDDATA_RATEDEVIATION_STARTINDEX + 0U] = 0x7DU;
        ...
        StbM_SharedData[timeBaseStartIndex + STBM_SHAREDDATA_RATECORRECTION_STARTINDEX + 0U] = 0x10U;
    ...
    StbM_SharedData[STBM_SHAREDDATA_TIMEBASENUMBER_STARTINDEX + 0U] = (uint8)((uint32)(STBM_NUMBER_TIME_BASES_THAT_SHARE_DATA_MODULES & STBM_32BIT_MASK1) >> 8U);
...
)
{
  ...
  for (j = 0U; j < STBM_NUMBER_OF_SYNC_CANTSYN_MASTER_TIME_RECORD_BLOCKS; j++)
  {
    ...
    StbM_CanMasterTimingData[j].segmentId = STBM_INVALID_SEGMENTID;
  }
...
)
{
  ...
  for (j = 0U; j < STBM_NUMBER_OF_SYNC_CANTSYN_SLAVE_TIME_RECORD_BLOCKS; j++)
  {
    ...
    StbM_CanSlaveTimingData[j].segmentId = STBM_INVALID_SEGMENTID;
  }
...
)
{
  ...
  for (j = 0U; j < STBM_NUMBER_OF_SYNC_FRTSYN_MASTER_TIME_RECORD_BLOCKS; j++)
  {
    ...
    StbM_FrMasterTimingData[j].segmentId = STBM_INVALID_SEGMENTID;
    StbM_FrMasterTimingData[j].currentCycle = STBM_INVALID_CURRENTCYCLE;
    StbM_FrMasterTimingData[j].currentMacroticks = STBM_INVALID_CURRENTMACROTICKS;
    StbM_FrMasterTimingData[j].macrotickDuration = STBM_INVALID_MACROTICKDURATION;
    StbM_FrMasterTimingData[j].cycleLength = STBM_INVALID_CYCLELENGTH;
  }
...
)
{
  ...
  for (j = 0U; j < STBM_NUMBER_OF_SYNC_FRTSYN_SLAVE_TIME_RECORD_BLOCKS; j++)
  {
    ...
    StbM_FrSlaveTimingData[j].segmentId = STBM_INVALID_SEGMENTID;
    StbM_FrSlaveTimingData[j].currentCycle = STBM_INVALID_CURRENTCYCLE;
    StbM_FrSlaveTimingData[j].CurrentMacroticks = STBM_INVALID_CURRENTMACROTICKS;
    StbM_FrSlaveTimingData[j].FCNT = 0U;
    StbM_FrSlaveTimingData[j].macrotickDuration = STBM_INVALID_MACROTICKDURATION;
    StbM_FrSlaveTimingData[j].cycleLength = STBM_INVALID_CYCLELENGTH;
...
)
{
  ...
  StbM_FrTimeMasterMeasurementType lcl_StbM_FrMasterTimingData[STBM_NUMBER_OF_SYNC_FRTSYN_MASTER_TIME_RECORD_BLOCKS] = {{0U, {0U, 0U}, {0U, 0U}, 0U, 0U, 0U, 0U, 0U}};
...
STATIC FUNC(void, STBM_CODE)  StbM_Check_Pending_Clone_Request(uint8 srcIndex)
{
  ...
  for( destIndex = 0; destIndex < (STBM_NUMBER_OF_SYNC_TIMEBASES + STBM_NUMBER_OF_OFFSET_TIMEBASES + STBM_NUMBER_OF_PURE_TIMEBASES) ; destIndex++ )
  {
     ...
     if(srcTimeIndex == srcIndex)
     {
         /* !LINKSTO StbM.SWS_StbM_00532,1 */
         /*Do the clone between srcIndex  & destIndex */
         (void)StbM_CloneTimeBase_Internal( srcTimeIndex ,destIndex, srcCloneCfg );
         SchM_Enter_StbM_SCHM_STBM_EXCLUSIVE_AREA_0();
...
)
{
...
#endif /* #if (STBM_DEV_ERROR_DETECT == STD_ON) */
  {
    ...
    if(E_OK == StbM_UpdateRxTimeTuple(timeBaseId, &currentTimeStamp, &receivedTimeTupleVLT))
    {
      ...
      {
        STBM_FLAG_SET(tmpStatusNotification, STBM_EV_TIMEOUT_REMOVED);
      }
      if((STBM_FLAG_IS_SET(flags, STBM_SYNC_TO_GATEWAY_FLAG)) && (!(currentTimeStamp.timeBaseStatus & STBM_SYNC_TO_GATEWAY_FLAG)))
      {
         /* Transition from 1 to 0. */
         /* !LINKSTO StbM.SWS_StbM_00284.EV_SYNC_TO_GLOBAL_MASTER,1 */
         STBM_FLAG_SET(tmpStatusNotification, STBM_EV_SYNC_TO_GLOBAL_MASTER);
      }
      if((STBM_FLAG_IS_SET(currentTimeStamp.timeBaseStatus, STBM_SYNC_TO_GATEWAY_FLAG)) && (!(flags & STBM_SYNC_TO_GATEWAY_FLAG)))
      {
        /* Transition from 0 to 1. */
        /* !LINKSTO StbM.SWS_StbM_00284.EV_SYNC_TO_SUBDOMAIN,1 */
        STBM_FLAG_SET(tmpStatusNotification, STBM_EV_SYNC_TO_SUBDOMAIN);
      }
#endif /* STBM_STATUS_NOTIFICATIONS_USED == STD_ON */
...
)
{
...
#endif /* (STBM_SHARED_DATA_VERSION == STBM_SHARED_DATA_V2) */
#endif /* (STBM_TIME_BASES_THAT_SHARE_DATA_USED == STD_ON) */
  DBG_STBM_CALCULATELOCALTIMEANDUPDATE_ENTRY(timeBaseId, timeStampPtr);
  ...
  DBG_STBM_CALCULATELOCALTIMEANDUPDATE_EXIT(timeBaseId, timeStampPtr);
}
...
)
{
  ...
  DBG_STBM_PERFORMSLAVERATECORRECTION_ENTRY(timeBaseId);
  ...
  DBG_STBM_PERFORMSLAVERATECORRECTION_EXIT(timeBaseId);
...
)
{
  ...
  DBG_STBM_CALCULATESLAVERATECORRECTION_ENTRY(rcDataIndex, commonIndex);
  ...
  DBG_STBM_CALCULATESLAVERATECORRECTION_EXIT(rcDataIndex, commonIndex);
}
...
)
{
  ...
  DBG_STBM_HANDLESLAVERATECORRECTION_ENTRY(commonIndex);
  ...
  DBG_STBM_HANDLESLAVERATECORRECTION_EXIT(commonIndex);
}
...
)
{
  ...
  DBG_STBM_CALCULATEANDRECORDRATECORRECTION_ENTRY(commonIndex);
  ...
  DBG_STBM_CALCULATEANDRECORDRATECORRECTION_EXIT(commonIndex);
}
#endif /* STBM_SLAVE_RATE_CORRECTION_USED == STD_ON */
...
)
{
  ...
  uint8 rcDataIndex = STBM_GET_SLAVE_RATEDATA_TIMEBASE_INDEX(commonIndex);
#endif /* STBM_SLAVE_RATE_CORRECTION_USED == STD_ON */
  DBG_STBM_UPDATERXTIMETUPLE_ENTRY(timeBaseId, globalStampPtr, virtualTimePtr);
  ...
  if(!(STBM_TIMEBASE_ID_IS_OFFSET(timeBaseId)))
  {
    ...
    if(retval == E_OK)
    {
      ...
      /* !LINKSTO StbM.SWS_StbM_00529.SyncRxTimeTuple.CalcTfrd,1 */
      StbM_SubtractVirtualLocalTimesToTimeStamp
      (
        &currentTvSync,
  ...
  DBG_STBM_UPDATERXTIMETUPLE_EXIT(retval, timeBaseId, globalStampPtr, localTimePtr);
...
)
{
...
#endif /* (STBM_DEV_ERROR_DETECT == STD_ON) */
  {
    ...
    if(srcTimeBaseId != STBM_NO_SOURCE_TIME_BASE)
    {
        ...
        SchM_Enter_StbM_SCHM_STBM_EXCLUSIVE_AREA_0();
        srcTimeBaseStatus  = StbM_TimeStamps[srcIndex].globalTime.timeBaseStatus;
        SchM_Exit_StbM_SCHM_STBM_EXCLUSIVE_AREA_0();
        if( ( srcTimeBaseStatus & statusMask) == statusValue)
        {
    ...
    #endif /*(STBM_TIMEOUTS_USED == STD_ON)*/
            {
               ...
               else
               {
                  ...
                  /* !LINKSTO StbM.SWS_StbM_00530.CloneTimeBase.DEFERRED_COPY_flag.Not_Set,1 */
        ...
        else
        {
            ...
            /* Reject the clone request should*/
        }
    }
    else
    {
