...
)
{
  ...
  /* stop running timer */
  if((ChannelPtr->State == CANTP_CH_TX_WAITTMR_CF)||(ChannelPtr->State == CANTP_CH_TX_REQUESTEDBUF_CF))
  {
  ...
  /* check if there is a CF postponed */
  if (ChannelPtr->PostRxIndFrameType == CANTP_N_PCI_TYPE_CF)
  {
    /* clear postpone flag */
