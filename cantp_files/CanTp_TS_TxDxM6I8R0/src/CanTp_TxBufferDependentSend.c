      ...
      switch(RetVal_DataReq)
      {
        case BUFREQ_OK:
        {
          ...
          if( Ret_Postponed == TRUE)
          {
            ...
            if((ChannelPtr->State == CANTP_CH_TX_BUFREQ_SF)||(ChannelPtr->State == CANTP_CH_TX_BUFREQ_FF))
            {
...
)
{
  ...
  for(idx = CANTP_MAX_TX_CHANNELS; idx < CANTP_MAX_PARALLEL_CHANNELS; idx++)
  {
    ...
    /* Check if locked channels, were already locked for the current RxNSduId AND
    ...
    if((CANTP_CFG_GET_RXNSDU_CONFIG(parallelChPtr->NSduId).NPduConfIdFc == CanTpTxPduId) &&
      ((CANTP_CH_GET_ACTION(parallelChPtr->State) == CANTP_CH_ACTION_POSTPONE_FRAME) ||
       (CANTP_CH_GET_ACTION(parallelChPtr->State) == CANTP_CH_ACTION_CONFIRM)))
      {
