...
/** \brief CanTp_CheckSF_Length()
 **
 ** This function check the SF_DL against CAN_DL of a single frame when CAN FD is used.
 ** Table 13 from ISO_15765
 ** Default return value set to TRUE so if CAN_DL is invalid, frame will be invalidated in function CanTp_CheckNPduLength.
 **
 ** \param[in] Can_DL  CAN_DL of the frame
 ** \param[in] DataLength  SF_DL of the frame
 ** \param[in] StartPosition_SF_N_PCI  addressing type - standard or mixed/extended
 */
STATIC FUNC(boolean, CANTP_CODE) CanTp_CheckSF_Length
(
  PduLengthType Can_DL,
  uint8 DataLength,
  uint8 StartPosition_SF_N_PCI
);
...
#endif
{
    ...
    if(CanTp_FindNSduId(CanTpRxPduId, CanTpRxPduPtr->SduDataPtr, &NSduId, &FrameType))
    {
...
#endif /* CANTP_PARALLEL_CHANNELS_ENABLED == STD_ON */
      {
        ...
        {
          NPduMaxLength = CANTP_CFG_GET_RXNSDU_CONFIG(NSduId).NPduLength;
        }
...
#if (CANTP_FIXED_NPDU_LENGTH_SUPPORT == STD_OFF)
...
#endif
        {
          ...
          if(IgnoreMsgFlag == (boolean)FALSE)
          {
            ...
            if(
                ...
                ((FrameType == CANTP_N_PCI_TYPE_FF) && ((CANTP_CHECK_FLAG(ChannelPtr->ConnectionFlags,CANTP_TXLASTFRAME_MASK)) != (boolean)FALSE))))
            )
            {
...
)
{
  ...
  else if((CanTpRxPduPtr->SduDataPtr[StartPosition_SF_N_PCI] & CANTP_N_PCI_SF_DL_MASK) == 0U) /* low nibble SF_DL is 0 and CAN_DL > 8 => CAN FD single frame */
  {
    ...
    {
      /* Check SF_DL */
      ValidFrameLength = CanTp_CheckSF_Length(CanTpRxPduPtr->SduLength,DataLength,StartPosition_SF_N_PCI);
    }
  }
#endif
  else
  ...
  {
    /* Check CAN_DL */
    if(!CanTp_CheckNPduLength(CanTpRxPduPtr->SduLength, RxNSduCfgPtr->PaddingActivation, RxNSduCfgPtr->NPduLength))
    {
      ValidFrameLength = FALSE;
    }
    else
...
)
{
...
#endif /* CANTP_CANFD_ENABLED == STD_ON */
  {
     ...
     else if (MessageLength == 0U)
    {
      ...
      /* !LINKSTO CanTp.ISO2016.InvalidFF_DL, 1 */
      if((PduLenghTypeMax >= MessageLength_BigData) && (MessageLength_BigData > 4095U))
      {
...
)
{
  ...
  CANTP_PRECONDITION_ASSERT((DataLength > 0U), CANTP_INTERNAL_API_ID);
    ...
    else if(!CanTp_CheckNPduLength(CanTpRxPduPtr->SduLength, RxNSduCfgPtr->PaddingActivation, RxNSduCfgPtr->NPduLength))
    {
      ValidFrameLength = FALSE;
    }
    else
    ...
    case CANTP_CH_TX_CONFIRM_CF:
    {
      ...
      if(ExpectedFrame)
      {
        ...
        case CANTP_N_PCI_FC_CTS:
        {
          ...
          if(ChannelPtr->State == CANTP_CH_TX_RECEIVE_FC)
          {
            DBG_CANTP_CHANNEL_STATE_GRP(
                DBG_CANTP_GET_CHANNEL_INDEX(ChannelPtr),
                ChannelPtr->State,
...
{
  /* Default return value set to TRUE so if CAN_DL is invalid, frame will be invalidated in function CanTp_CheckNPduLength. */
  boolean ValidSF_DL = TRUE;
  switch(Can_DL)
  {
    case CANTP_FD_NPDU_LENGTH_12:
    {
      if(((StartPosition_SF_N_PCI == CANTP_SF_STARTPOS_NPCI_STD) && (SF_DL_8 <= DataLength) && (DataLength <= SF_DL_10)) ||  /* STANDARD/NORMAL_FIXED addressing*/
         ((StartPosition_SF_N_PCI == CANTP_SF_STARTPOS_NPCI_EXTMIX) && (SF_DL_7 <= DataLength) && (DataLength <= SF_DL_9)))     /* EXTENDED OR MIXED addressing*/
      {
        ValidSF_DL = TRUE;
      }
      else
    ...
    case CANTP_FD_NPDU_LENGTH_16:
    {
      if(((StartPosition_SF_N_PCI == CANTP_SF_STARTPOS_NPCI_STD) && (SF_DL_11 <= DataLength) && (DataLength <= SF_DL_14)) ||  /* STANDARD/NORMAL_FIXED addressing*/
         ((StartPosition_SF_N_PCI == CANTP_SF_STARTPOS_NPCI_EXTMIX) && (SF_DL_10 <= DataLength) && (DataLength <= SF_DL_13)))     /* EXTENDED OR MIXED addressing*/
      {
        ValidSF_DL = TRUE;
      }
      else
    ...
    case CANTP_FD_NPDU_LENGTH_20:
    {
      if(((StartPosition_SF_N_PCI == CANTP_SF_STARTPOS_NPCI_STD) && (SF_DL_15 <= DataLength) && (DataLength <= SF_DL_18)) ||  /* STANDARD/NORMAL_FIXED addressing*/
         ((StartPosition_SF_N_PCI == CANTP_SF_STARTPOS_NPCI_EXTMIX) && (SF_DL_14 <= DataLength) && (DataLength <= SF_DL_17)))     /* EXTENDED OR MIXED addressing*/
      {
        ValidSF_DL = TRUE;
      }
      else
    ...
    case CANTP_FD_NPDU_LENGTH_24:
    {
      if(((StartPosition_SF_N_PCI == CANTP_SF_STARTPOS_NPCI_STD) && (SF_DL_19 <= DataLength) && (DataLength <= SF_DL_22)) ||  /* STANDARD/NORMAL_FIXED addressing*/
         ((StartPosition_SF_N_PCI == CANTP_SF_STARTPOS_NPCI_EXTMIX) && (SF_DL_18 <= DataLength) && (DataLength <= SF_DL_21)))     /* EXTENDED OR MIXED addressing*/
      {
        ValidSF_DL = TRUE;
      }
      else
    ...
    case CANTP_FD_NPDU_LENGTH_32:
    {
      if(((StartPosition_SF_N_PCI == CANTP_SF_STARTPOS_NPCI_STD) && (SF_DL_23 <= DataLength) && (DataLength <= SF_DL_30)) ||  /* STANDARD/NORMAL_FIXED addressing*/
         ((StartPosition_SF_N_PCI == CANTP_SF_STARTPOS_NPCI_EXTMIX) && (SF_DL_22 <= DataLength) && (DataLength <= SF_DL_29)))     /* EXTENDED OR MIXED addressing*/
      {
        ValidSF_DL = TRUE;
      }
      else
    ...
    case CANTP_FD_NPDU_LENGTH_48:
    {
      if(((StartPosition_SF_N_PCI == CANTP_SF_STARTPOS_NPCI_STD) && (SF_DL_31 <= DataLength) && (DataLength <= SF_DL_46)) ||  /* STANDARD/NORMAL_FIXED addressing*/
         ((StartPosition_SF_N_PCI == CANTP_SF_STARTPOS_NPCI_EXTMIX) && (SF_DL_30 <= DataLength) && (DataLength <= SF_DL_45)))     /* EXTENDED OR MIXED addressing*/
      {
        ValidSF_DL = TRUE;
      }
      else
    ...
    case CANTP_FD_NPDU_LENGTH_64:
    {
      if(((StartPosition_SF_N_PCI == CANTP_SF_STARTPOS_NPCI_STD) && (SF_DL_47 <= DataLength) && (DataLength <= SF_DL_62)) ||  /* STANDARD/NORMAL_FIXED addressing*/
         ((StartPosition_SF_N_PCI == CANTP_SF_STARTPOS_NPCI_EXTMIX) && (SF_DL_46 <= DataLength) && (DataLength <= SF_DL_61)))     /* EXTENDED OR MIXED addressing*/
      {
        ValidSF_DL = TRUE;
      }
      else
    ...
    default: 
    {
      /* do nothing, frame will be invalidated in function CanTp_CheckNPduLength() */
      break;
    }
  }
  return ValidSF_DL;
