...
#endif /*#if(CANTP_DISTRIBUTED_CHANNEL_SUPPORT == STD_ON)*/
    ...
    */
    ...
    /* initialize ( = stop) each connection channel */
      ...
      for(index = 0; index < CANTP_NUMBER_CONFIGURED_CHANNELS; index++)
      {
        ...
        {
          CanTp_Channel[tx_channel].Partition = partition;
        }

        if(rx_channel != INVALID_RX_CHANNEL_ID)
        {
          CanTp_Channel[rx_channel].Partition = partition;
        }
      }
#endif
...
#else
  if(0 != CanTp_NonIdleChannelArray[ChannelPtr->Partition])
  {
