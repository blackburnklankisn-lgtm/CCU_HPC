...
#if (defined CANTP_CFG_GET_CONFIGURED_PARTITION) /* to prevent double declaration */
#error CANTP_CFG_GET_CONFIGURED_PARTITION is already defined
#endif /* if (defined CANTP_CFG_GET_CONFIGURED_PARTITION) */

#define CANTP_CFG_GET_CONFIGURED_PARTITION(Index)   \
    (CANTP_PBCFG_ACCESS(CanTp_ChannelsToInternalChannelIDType, CanTp_CfgPtr->ConfiguredChannelsPtr)[(Index)].ReferredByPartition)
    
#if (defined CANTP_CFG_GET_RXTPCHANNEL_FOR_PARTITION) /* to prevent double declaration */
#error CANTP_CFG_GET_RXTPCHANNEL_FOR_PARTITION is already defined
#endif /* if (defined CANTP_CFG_GET_RXTPCHANNEL_FOR_PARTITION) */

#define CANTP_CFG_GET_RXTPCHANNEL_FOR_PARTITION(Index)   \
    (CANTP_PBCFG_ACCESS(CanTp_ChannelsToInternalChannelIDType, CanTp_CfgPtr->ConfiguredChannelsPtr)[(Index)].Rx_Channel)

#if (defined CANTP_CFG_GET_TXTPCHANNEL_FOR_PARTITION) /* to prevent double declaration */
#error CANTP_CFG_GET_TXTPCHANNEL_FOR_PARTITION is already defined
#endif /* if (defined CANTP_CFG_GET_TXTPCHANNEL_FOR_PARTITION) */

#define CANTP_CFG_GET_TXTPCHANNEL_FOR_PARTITION(Index)   \
    (CANTP_PBCFG_ACCESS(CanTp_ChannelsToInternalChannelIDType, CanTp_CfgPtr->ConfiguredChannelsPtr)[(Index)].Tx_Channel)
[!ENDIF!]
...
[!IF "CanTpGeneral/CanTpDedicatedChannelProcessingSupport = 'true'"!]
/** \brief CanTp array for checking if channels of specific partition are idle or not (0 means all channels are IDLE) */
#if((CANTP_MAX_TX_CHANNELS + CANTP_MAX_RX_CHANNELS) > 255U)
  #define CANTP_START_SEC_VAR_CLEARED_16
  #include <CanTp_MemMap.h>

extern VAR(uint16, CANTP_VAR) CanTp_NonIdleChannelArray[CANTP_MAIN_PARTITION + 1];
...
extern VAR(uint8, CANTP_VAR) CanTp_NonIdleChannelArray[CANTP_MAIN_PARTITION + 1];
